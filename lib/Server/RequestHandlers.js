/*
 * ChappySinclair
 * https://github.com/jrm/ChappySinclair
 *
 * Copyright (c) 2013 James Meldrum
 * Licensed under the MIT license.
 */

'use strict';
var _ = require( 'lodash' );
var ResponseData = function( status, header, body){

  this.status = status;
  this.header = header;
  this.body = body;

};

var GenericHandler = function( pathName, request, postData, response, success ){

  this.pathName = pathName;
  this.request = request;
  this.postData = postData;
  this.response = response;
  this.success = success;

};

GenericHandler.prototype.respond = function( responseData ){

  console.log( 'responding', responseData, this.response );
  
  this.response.writeHead( responseData.status, responseData.head );
  this.response.write( responseData.body );
  this.response.end();

  this.success();

};

var list = function ListHandler( pathName, request, postData, response, success ){

  console.log( 'list called' );
  GenericHandler.call( this, pathName, request, postData, response, success );
  GenericHandler.prototype.respond.call( this, new ResponseData( '200', 'header', 'list.body' ));

},

_default = function DefaultHandler( pathName, request, postData, response, success ){

  console.log( 'default called' );
  GenericHandler.call( this, pathName, request, postData, response, success );
  GenericHandler.prototype.respond.call( this, new ResponseData( '200', 'header', 'default.body' ));

},

run = function RunHandler( pathName, request, postData, response, success ){

  GenericHandler.call( this );
  GenericHandler.call( this, pathName, request, postData, response, success );
  GenericHandler.prototype.respond.call( this, new ResponseData( '200', 'header', 'run.body' ));

},

logs = function LogsHandler( pathName, request, postData, response, success ){

  GenericHandler.call( this, pathName, request, postData, response, success );
  GenericHandler.prototype.respond.call( this, new ResponseData( '200', 'header', 'logs.body' ));

},

ws = function WsHandler( pathName, request, postData, response, success ){

  GenericHandler.call( this, pathName, request, postData, response, success );
  GenericHandler.prototype.respond.call( this, new ResponseData( '200', 'header', 'ws.body' ));

};
  

// Extend all methods from GenericHandler

  _.each([ list, run, logs, ws, _default ], function( handler ){
  
    var handler_old = handler;
  
    handler.prototype = Object.create( GenericHandler.prototype );
    handler.prototype.constructor = handler_old;
  
  });

exports.list    = list;
exports.run     = run;
exports.logs    = logs;
exports.ws      = ws;
exports.default = _default;
